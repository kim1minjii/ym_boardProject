	...
	<div id="bodywrap" class="container">
		...
		<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${aList}" var="dto">
					<tr>
						<td>${dto.num}</td>
						<td><c:url var="path" value="fview.do">
		...
		<div class="pagelist text-center">
			...
			<c:if test="${pv.startPage>1}">
				<a href="flist.do?currentPage=${pv.startPage-pv.blockPage}">이전</a>
			</c:if>
			<!-- 이전 출력 끝 -->
			...
			<c:if test="${pv.endPage<pv.totalPage}">
				<a href="flist.do?currentPage=${pv.startPage+pv.blockPage}">다음</a>
			</c:if>
			<!-- 다음 출력 끝 -->
		</div>
		<!-- 페이징 -->
			...
			<div class="container">
				<div class="col text-end">
					<button class="btn btn-light row justify-content-end" type="button"
						onclick="location.href='/myapp/fwrite.do'">글쓰기</button>
				</div>
			</div>
		</div>
	</div>
